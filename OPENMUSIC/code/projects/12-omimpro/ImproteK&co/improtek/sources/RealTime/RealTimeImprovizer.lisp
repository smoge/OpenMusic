...
(defclass* realTime-improvizer (improvizer)
   (
    ...
    ;(in general, next steps will be quicker <------------ Cékoissa ?????????????
    (nextPrefixImpactFound :initform nil :initarg :nextPrefixImpactFound :accessor nextPrefixImpactFound)
    ;MAJCONT 15/10/13 : plus juste un entier (indice) mais une liste (indiceassocié continuitéassociée)
...
(defmethod Improvize&send-groupsAnte-loop-next-factor ((self realTime-improvizer) &optional (scenario nil) (beatduration t) (beatIdxInImpro t) (hostsend t) (portsend t) (adresssend simple-base-string) (numAntescofo t))
  #|
  ; Ne pas confondre
...
|#
  ...
  (let* ((beatlistImpro (Improvize-loop-next-factor self scenario beatduration beatIdxInImpro)); hostsend portsend adresssend))
    ...
    (send-beatlist-as-string-group-for-antescofo beatlistImpro beatduration beatIdxInImpro hostsend portsend adresssend numAntescofo)
    ...
    ;            (progn
    ;              (setf beatforAntescofo (string-group-for-antescofo beat beatduration numAntescofo)); "string-group...." = celui dans "AntescofoTR.lisp" ???
...
; (pour gérer notes tenues, évènements syncopés...)
;(defmethod Improvize-loop-next-factor ((self realTime-improvizer) &optional (scenario nil) (beatduration t) (beatIdxInImpro t)); (hostsend t) (portsend t) (adresssend simple-base-string)) ;(numAntescofo t))
(defmethod Improvize-loop-next-factor ((self realTime-improvizer) (scenario t) (beatduration t) (beatIdxInImpro t)); (hostsend t) (portsend t) (adresssend simple-base-string)) ;(numAntescofo t))
  #|
  ; Ne pas confondre
  ; --> cas où au début on attaque directement sur ImproIdx > 1 (par exemple parce qu'on attendait que la mémoire soit déjà un peu remplie)
  ; --> cas où on revient en arrière sur une partie d'impro déjà calculée.
...
|#
  ...
  (let* ((beatlistImpro (Improvize-by-fragments self (list-length scenario) scenario))
    ...
    ;(send-beatlist-as-string-group-for-antescofo beatlistImpro beatduration beatIdxInImpro hostsend portsend adresssend simple-base-string numAntescofo)
    beatlistImpro))



; Quand generation query, test si sur partie déjà calculée (donc réécriture) 
...
; -----------------------------------------------------
(defmethod Improvize-by-fragments ((self realTime-improvizer) (length t) &optional (scenario nil))
  (let ((impro nil) (current-grid-suffix nil) (next-grid-suffix nil))  
    ...
    (when scenario (setf next-grid-suffix scenario))
    ...
    (loop for i from 1 to (min length (maxetat self)) ; avant : juste length
          ...
          (setf next-grid-suffix scenario)
...
(defmethod navigate-realtime ((self realTime-improvizer) (NavigationOrder list) (beatduration t) (hostsend t) (portsend t) (adresssend simple-base-string) &optional numAntescofo)
  (let* ((suffixgrid (nth 1 NavigationOrder))
         (beatIdxInImpro (nth 0 NavigationOrder)))
    (if (otext self 10)
        (let ((numAnte 1))
          (when numAntescofo (setf numAnte numAntescofo))
      ...
      ;(format *om-stream* "******YA PAS DE TRUCS -> JE NAVIGUE TROP APS******~%")
      )))
...
;---------------------------------------------------------------------------------
(defmethod Improvize-groupAnte-next-state ((self realTime-improvizer)  &optional (scenario nil) (beatduration t) (numAntescofo t))
