...
(defmethod ImprovizeOnHarmGrid ((self improvizer) (length integer) (harmGrid list))
  (ImprovizeOnScenario self length harmGrid))
...
(defmethod select-matching-prefixes ((self improvizer) (grid list))
  (let* (
         (MP (MorrisPratt_prefixes self grid)) 
         (found-grid-prefixes (nth 0 MP)) 
         (length_longest (nth 1 MP)) 
         (length_longest (nth 1 MP)) 
         (selected-prefixes nil))
    (if (= *print_info_find* 1) (format *om-stream* "(((Selecting prefixes ((according to constraints)) of ~a)))~%" grid)) 
    (list  
    ...
    (let* ((Transpos (AuthorizedTranspos self))
      ...
      (if (bestTranspoMode self)
          (let* ((best_transp_len 0) (best_transp_idxs nil) (best_delta 0)
            ...
            (loop for delta in (om- intervalTranspo (CurrentTranspo self)) 
                  ...
                  (if (= *print_info_find* 1) 
                      (format *om-stream* "[Searching for BEST transpo of ~a]~%[with transpo = ~D -> transp-grid = ~a]~%" grid delta (TransposeGrid grid delta)))
        ...
        (loop for delta in (om- intervalTranspo (CurrentTranspo self))
              do 
              (if (= *print_info_find* 1) 
                      (format *om-stream* "~%~%~%[Searching for FIRST RANDOM transpo of ~a]~%[with transpo = ~D -> transp-grid = ~a]~%" grid delta (TransposeGrid grid delta)))
