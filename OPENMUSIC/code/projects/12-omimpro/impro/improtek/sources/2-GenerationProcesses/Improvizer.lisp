...
(defmethod display-infolabel ((self improvizer) scen)
    ...
    (if *print-navig-basics* 
        (progn 
          (format *om-stream* "label=~a (current transpo = ~a)" (FormatLabel (car scen)) (CurrentTranspo self))
          (if (= (CurrentTranspo self) 0) (format *om-stream* ", ") (format *om-stream* ", oracle=~a, " (FormatLabel (TransposeCloneLabel (car scen) (- (CurrentTranspo self))))))))
