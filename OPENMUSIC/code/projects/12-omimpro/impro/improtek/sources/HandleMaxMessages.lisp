...
(defun handle-messages-from-max (message)
  ...
  (cond
   ...
   ((string= (car message) "/launchTR")
    ...
    (LaunchRealtimeImpro *current-tune* host_server 3008 "/OM/improbeats" 7415 "/learnforTR" 7416 "/antescofo/improvize_next_steps")
    )

   ;End realtime (Tune.lisp)
   ;----------------------------
   ((string= (car message) "/killTR")
    ;(end-realtime *current-tune*)
    (if (RealtimeImprovizer-in-OSC-env *current-tune*) (end-realtime (RealtimeImprovizer-in-OSC-env *current-tune*)))
    )



   ;;;;;================================================================;;;;;;
   ;; Késseucé ça ????? C'est pas déjà dans le champ de la classe tune ???? ;;
   ;;;;;================================================================;;;;;;

   ; Improvize-groupAnte-next-state... n'existe plus !!! Est commenté dans real-time... voir ce que c'est devenu !!
   ; Où ça a été dispatché !!! (En espérant que ça le soit...)
   ...
   ( (string= (car message) "/set-max-continuity-oraclechan") 
     ...
     (progn
       ...
       (if (= (nth 2 message) *num-realtime-improvizer*)
           (setf (max-continuity (sharedData (RealTimeSystem (RealtimeImprovizer-in-OSC-env *current-tune*)))) (car (cdr message)))
   ...
   ;toOM3 - Calls "generate-polychan-impro-for-antescofo " (Tune.lisp)
   ;---------------------------------------------------------------------
     ...
     (progn
       ...
       (if (= (nth 1 message) *num-realtime-improvizer*)
           (let ((l (cdddr (reverse (multiple-value-list (get-decoded-time))))))
              (save-improvizer 
               (sharedData (RealTimeSystem (RealtimeImprovizer-in-OSC-env *current-tune*)))
               (format nil "~a/~a" 
   ...
   ( (string= (car message) "/set-start-region-oraclechan") 
     ...
     (if (= (nth 3 message) *num-realtime-improvizer*)
         (setf (start-region (sharedData (RealTimeSystem (RealtimeImprovizer-in-OSC-env *current-tune*)))) 
               (list (floor (* (nth 1 message) (/ (max 0 (1- (maxetat (sharedData (RealTimeSystem (RealtimeImprovizer-in-OSC-env *current-tune*)))))) 127))) 
                     (floor (* (nth 2 message) (/ (max 0 (1- (maxetat (sharedData (RealTimeSystem (RealtimeImprovizer-in-OSC-env *current-tune*)))))) 127)))))
         ...
         (progn
           ...
           (if (= (nth 1 (cdr message)) *num-realtime-improvizer*)
               (if (probe-file (nth 0 (cdr message)))
                   (setf (sharedData (RealTimeSystem (RealtimeImprovizer-in-OSC-env *current-tune*))) (load-realtimeImprovizer-fromSavedImprovize (nth 0 (cdr message))))
   ...
   ( (string= (car message) "/reset-tabou-oraclechan") 
     (progn
       ; /!\ Ghost of channel *num-realtime-improvizer* is used for the realtime improvizer
       (if (= (nth 1 message) *num-realtime-improvizer*)
           (ResetTabou (sharedData (RealTimeSystem (RealtimeImprovizer-in-OSC-env *current-tune*))))
