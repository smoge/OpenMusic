...
(defun handle-messages-from-max (message)
  ...
  (cond
   ...
   ; Modifie variable surveillée dans MultithreadsTR.lisp
   ;---------------------------------------------------------------------
   ...
   ;=====================================LANCER TR==================================
   ; TEST !
   ; Lance le mulithreading défini dans MultithreadsTR.lisp
   ; /!\ JUSTE TEST AVEC ORACLE 8... /!\
   ;---------------------------------------------------------------------
   ( (string= (car message) "/launchTR")
     (launch-realtime-on-oraclechan *current-tune* 8 host_server 3008 1)
     )
