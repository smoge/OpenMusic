...
(defmethod Improvize ((self improvizer) (length integer) &optional (harmgrid nil))
  ...
  ; JEROME 25/07/2013
  ; /!\ TODO : isoler cette boucle dans une fonction avec test d'arrêt en argument !!
  ; Maintenant qu'on travaille aussi en "temps-réel" on ne fera pas toujours nécessairement 
  ; d'un coup des parcours complets de cette boucle.
  ; PAS INTELLIGENT DE COPIER QUASIMENT LE MEME CODE DANS PLUSIEURS FONCTIONS !!!!!
...
(defmethod Improvize-play-next-state ((self improvizer)  &optional (harmgrid nil) (beatduration integer)) 
  (when harmgrid 
    ...
    ;Display info : /!\ Current-transpo is managed in "Improvize-one-step". 
    ;Transpositions here (label and current-grid-suffix) are only used to display info.        
    (if (not (= (CurrentTranspo self) 0)) (format *om-stream* "oracle=~a, " (FormatLabel (TransposeLabel (car harmgrid) (- (CurrentTranspo self)))) ))
