...
(defclass* realTime-improvizer (improvizer)
   (
    ...
    ;(lock :initform (mp:make-lock) :initarg :lock :accessor lock)
    ))
...
;------APPRENTISSAGE
; JEROME 03/09/13 :
; APPRENDRE UNE PETITE SEQUENCE N'EST PAS LONG...
; PLUTOT QU'A CHAQUE OBJET, FAIRE L'ATOMIC FUNCALL DE LA FONCTION CI-DESSOUS LA OU ELLE EST APPELEE ?? -> TEST !!
(defmethod learn-event-list ((self realTime-improvizer) (events list))
  (format *om-stream* "~%(START learning!)~%")
   (loop for event in events do 
         (progn
         #|
         "ATOMIC" remonté d'un cran : dans l'appel de cette fonction (learn-realtime)
         (sys:setup-atomic-funcall
          'ajouter-objet 
          self 
          event)
...
(defmethod Improvize-by-fragments ((self realTime-improvizer) (length integer) &optional (harmgrid nil))
  (let ((impro nil) (current-grid-suffix nil) (next-grid-suffix nil))  
    ...
    (loop for i from 1 to (min length (maxetat self)) ; avant : juste length
          ...
          ;===========!!!===========
          ; diff "3/2" avec Improvize ----> LOCK POUR NE PAS AVOIR DE PROBLEME AVEC ECRITURE ET LECTURE SIMULTANEE !!!!!
          ;===========!!!=========== 
...
(defmethod navigate-real-time ((self realTime-improvizer) (messagefornavigation-symbol t) (beatduration integer) (host t) (port_ante integer) (numAntescofo integer))
  (loop
   ...
   (mp:process-wait "Waiting for a grid suffix to navigate" 
                    (lambda () 
                      ;/!\ ((((MAUVAISE)))) SOLUTION TEMPORAIRE POUR NE PAS NAVIGUER SUR UN ORACLE VIDE
                      (if (otext self 1) 
                      ;/!\ ------------------------------------------------------------    
                          (symbol-value messagefornavigation-symbol))))
   (let ((message nil))
     (setf message (symbol-value messagefornavigation-symbol))
     ;SUFFIT POUR QUE CELA NE SE DECLENCHE PAS PLUSIEURS FOIS ???
     ; /!\ NON !!!! --------> D'OU CE IF MOCHE !!!
     (setf (symbol-value messagefornavigation-symbol) nil)
     (if message
         (let* ((suffixgrid (nth 1 message))
            ...
            (if beatlist 
                (progn
                  ...
                  (sys:setup-atomic-funcall 
                   ...
                   self 
                   beatlist)
                  (format *om-stream* "******* Length Beatlist : ~D~%-----> New maxetat : ~D~%" (list-length beatlist) (maxetat self))))
