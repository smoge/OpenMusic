...
(defmethod execute-event ((self om-event-engine) n)
  (cond ((= n EVT_HIGH_PRIORITY)
         (funcall (pop (om-event-engine-hp-queue self))))
...
(defmethod send-task-event-engine-top (priority (self function) &rest args)
  (cond ((= priority EVT_HIGH_PRIORITY)
         (push self (om-event-engine-hp-queue *om-event-engine*)))
        ((= priority EVT_MEDIUM_PRIORITY)
         (push self (om-event-engine-mp-queue *om-event-engine*)))
        ((= priority EVT_LOW_PRIORITY)
         (push self (om-event-engine-lp-queue *om-event-engine*)))
        (t nil))
  (when (om-event-engine-process *om-event-engine*)
    (mp:process-poke (om-event-engine-process *om-event-engine*))))

(defmethod send-task-event-engine-bottom (priority (self function) &rest args)
  (cond ((= priority EVT_HIGH_PRIORITY)
         (nconc (om-event-engine-hp-queue *om-event-engine*) (list self)))
        ((= priority EVT_MEDIUM_PRIORITY)
         (nconc (om-event-engine-mp-queue *om-event-engine*) (list self)))
        ((= priority EVT_LOW_PRIORITY)
         (nconc (om-event-engine-lp-queue *om-event-engine*) (list self)))
        (t nil))
  (when (om-event-engine-process *om-event-engine*)
    (mp:process-poke (om-event-engine-process *om-event-engine*))))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
